...
public class HubRequestService {
	...
	public <T extends HubView> T getItem(final HubRequest hubRequest, Class<T> clazz) throws HubIntegrationException {
	...
	public <T extends HubView> T getItem(String url, Class<T> clazz) throws HubIntegrationException {
		final HubRequest hubRequest = getHubRequestFactory().createGetRequest(url);
		return getItem(hubRequest, clazz);
	}

	public <T extends HubView> T getItem(final JsonObject jsonObject, final Class<T> clazz) {
		final T item = getRestConnection().getGson().fromJson(jsonObject, clazz);
		item.setJson(jsonObject.toString());
		return item;
	}

	public <T extends HubView> T getItem(final JsonElement jsonElement, final Class<T> clazz) {
