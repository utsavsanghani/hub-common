...
public class UserConfigTransform implements ItemTransform<List<UserConfigItem>, UserOptionLinkItem> {
	...
	public UserConfigTransform(final UserRequestService userRequestService, final ExtensionConfigRequestService extensionConfigRequestService) {
	...
	public List<UserConfigItem> transform(final UserOptionLinkItem item) throws HubIntegrationException {
		final UserView user = userRequestService.getItem(item.getUser());
		if (!user.getActive()) {
